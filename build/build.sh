...
function build() {
  ...
  go build -mod=mod -buildmode=pie -ldflags "-s -extldflags=-Wl,-z,now  -X huawei.com/npu-exporter/v5/versions.BuildName=${OUTPUT_NAME} \
            -X huawei.com/npu-exporter/v5/versions.BuildVersion=${build_version}_linux-${arch}" \
    -o ${OUTPUT_NAME}
  ls ${OUTPUT_NAME}
  if [ $? -ne 0 ]; then
